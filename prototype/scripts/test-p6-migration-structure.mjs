...
assert.match(migration, /DROP POLICY catalog_events_p4_all[\s\S]*CREATE POLICY catalog_events_p4_all[\s\S]*rebuy[.]p6[.]op'[\s\S]*audit_list[\s\S]*rebuy[.]p6[.]organization_id[\s\S]*rebuy[.]p6[.]store_id/)
assert.match(migration, /DROP POLICY inventory_events_p4_all[\s\S]*CREATE POLICY inventory_events_p4_all[\s\S]*rebuy[.]p6[.]op'[\s\S]*audit_list[\s\S]*rebuy[.]p6[.]organization_id[\s\S]*rebuy[.]p6[.]store_id/)
assert.match(migration, /list_my_merchant_audit_impl[\s\S]*RETURN QUERY SELECT[\s\S]*UNION ALL[\s\S]*public[.]catalog_events[\s\S]*UNION ALL[\s\S]*public[.]inventory_events[\s\S]*ORDER BY r[.]created_at DESC, r[.]entity_id DESC LIMIT p_limit/)
assert.doesNotMatch(migration.match(/CREATE OR REPLACE FUNCTION private[.]list_my_merchant_audit_impl[\s\S]*?END\n[$]function[$];/)?.[0] ?? '', /jsonb_agg|jsonb_to_recordset|FOR v_listing/)
