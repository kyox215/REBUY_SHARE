...
文档状态：G2-A0 执行中；Entry 已授权，待独立安全审查与 Owner Exit Gate
...
2026-08-27 Owner 已签署 G1-19、G1 Exit=GO，并授权打开 G2-A0。当前只执行本地 docs-only 合同候选；G2-A0 Exit 仍待独立安全审查与 Owner Gate，G2-A1 保持未开始。授权 ref=`d51f1c7cb47e2fe2932b29bd39420f5d092a8160`，不授权 Supabase/Auth/DB/Storage/Realtime、secret/env、真实账号或任何部署/外部资源写入。
...
| A0-09 | MFA 候选边界：主 TOTP、不同设备/安全位置的备用 TOTP、受审计人工恢复；phone/SMS 与静态恢复码不预设（待 Owner 决定） | 若 Owner 采纳，备用因子必须在不同设备或安全位置；人工恢复需要身份核验、职责分离、AAL/会话重置、通知、限速和审计；support 不得单独批准；AAL2 范围/时点和双人复核仍待决定 | G2-A0 Owner Gate；A1 验证因子生命周期；A5 进行高风险流程和恢复专项审查 |
...
本节是独立只读安全预检的合同补充，不是自动生效的现行产品政策，也不代表已连接 Supabase/Auth、数据库、Storage 或任何外部项目。原记录日 2026-08-26 的 G1 Exit 快照为 NO-GO；2026-08-27 已由 Owner 以 G1-19/ref=`d51f1c7cb47e2fe2932b29bd39420f5d092a8160` 签署 G1 Exit=GO 并打开 G2-A0 docs-only 执行。G2-A0 Exit 仍需独立安全审查与 Owner 明确 Gate，G2-A1 保持未开始。完整记录见[G2-A0 Entry preflight 证据](./evidence/G2-A0/2026-08-26-entry-preflight/README.md)和[G2-A0 阶段记录](./stages/G2-A0-账号安全合同与威胁模型验收.md)。
...
| 主 TOTP 与备用 TOTP 因子元数据 | 极高敏感账户资料 | MFA 绕过、锁死、恢复滥用 | 候选控制为不同设备/安全位置、AAL2、重新认证、人工恢复职责分离；AAL2 范围/时点待 Owner 决定 |
...
| MFA 恢复滥用 | support 以工单理由替管理员解除 MFA，或主/备用因子同处一设备 | support 无批准权；若 Owner 采纳恢复候选，则需身份核验、职责分离、AAL/会话重置、通知审计；双人复核范围待 Owner 决定 | A1/A5 恢复演练和 Owner 决定后的复核证据 |
...
6. 若 Owner 采纳备用因子候选，备用因子必须与主因子处于不同设备或安全位置；人工恢复不能由单一 support 人员完成；AAL2 范围/时点和双人复核待 Owner 决定。
...
| MFA 人工恢复单点滥用 | 候选控制待 Owner 决定，未演练 | A1/A5 | 身份核验、职责分离、AAL/会话重置、通知、审计；双人复核按 Owner 决定记录 | 不允许 support 单独恢复 |
...
- MFA 备用 TOTP 不同设备/安全位置、人工恢复身份核验、职责分离、AAL/会话重置、通知和审计已形成候选合同；是否采纳、AAL2 范围/时点及双人复核仍待 Owner 决定。
...
1. 接受或修订本 ADR 的 `A0-01～A0-15` 及日期化 Entry 补充和上述安全不变量；未明确采纳的候选继续保持待决。
...
4. 必须完成独立安全审查，覆盖身份、隐私、Storage、RLS、跨租户、会话和威胁矩阵；按 05 的高风险规则，A4/A5 仍必须专项审查。
5. 确认 A0 验收证据已归档并由 Owner 明确签署后，才可把 A1 状态从“合同待执行”改为“准备中/待资源授权”；不得因 A0 Exit 直接开始 Auth 实测。
