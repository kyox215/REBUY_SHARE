...
workflow commit：`a388348b81300ca00f669d0bd62b0748b9f191a5`
...
| Node/pnpm | Node `22.12.0`；Corepack 驱动 pnpm `10.33.3`；`package-manager-cache: false` |
| 工作目录 | `defaults.run.working-directory: prototype`；所有应用命令在 `prototype` 执行 |
| 顺序 | Corepack/pnpm 版本检查 → `install --frozen-lockfile` → `typecheck` → `lint` → `build` |
...
- 从 workflow commit `a388348b81300ca00f669d0bd62b0748b9f191a5` 创建临时 archive 副本。
...
| 诊断 | 结果 | 边界 |
|---|---|---|
| Node22 自带版本 | Corepack `0.29.4` | 与 Node `22.12.0` 兼容；未修改全局 NVM 安装 |
| 无缓存临时 `COREPACK_HOME` | 解析 pnpm `10.33.3` 时因沙箱 registry DNS `ENOTFOUND registry.npmjs.org` 失败 | 归类为网络/沙箱限制，不是签名或版本兼容失败；未绕过签名 |
| 临时缓存复验 | 将已存在的 pnpm `10.33.3` 包缓存复制到临时 `COREPACK_HOME` 后，`corepack pnpm@10.33.3 --version` 输出 `10.33.3` | 证明 Corepack `0.29.4` 可安全调用目标版本；调用发生在临时 `prototype/` 目录 |
| 备选 Corepack `0.34.6` | 未采用 | 无需安装/全局替换；没有 `npm --global` 写入、`COREPACK_INTEGRITY_KEYS=0` 或签名跳过 |
| pnpm/action-setup | 未使用 | 版本由 Corepack 精确驱动 |

实际 workflow 只使用 runner 自带 Corepack 和显式 `corepack pnpm@10.33.3`；未来若 runner 的 Corepack 行为发生变化，须先做独立供应链审查，不在 workflow 中浮动到 `latest`。
...
| 顺序 | 命令 | 结果 | 最小摘要 |
|---|---|---:|---|
| 0 | `node --version` / `corepack --version` / `corepack pnpm@10.33.3 --version` | 0 | `v22.12.0` / `0.29.4` / `10.33.3` |
...
- `git diff` 相对基线 `f05c5f8375143909ae8e01d87b4267a321b590ad` 只新增 workflow；`prototype/**` 无差异。
...
若 workflow pin、触发器、权限或命令合同有误，删除或回退 workflow commit `a388348b81300ca00f669d0bd62b0748b9f191a5` 即可回到 `f05c5f8375143909ae8e01d87b4267a321b590ad`；不修改、不删除 G1.1 tag/ref。任何未来 G1.2b 远端失败只停止晋级并回退到上一可验证 ref，需另行 Owner 决策。
...
- Corepack 网络获取可能受 runner/网络策略影响；不得用签名绕过或浮动版本解决。若 `0.29.4` 在未来 runner 无法获取目标包，先记录失败，再按供应链审查决定是否精确引导 `0.34.6`。
- 本批只完成本地静态和本地等价证据；未运行真实 GitHub Actions、未验证 GitHub runner 触发/权限摘要、未配置 remote、未部署 Preview、未接 Supabase/Auth/数据库/生产。
