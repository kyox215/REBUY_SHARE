...
- 后续误操作事实：`vercel curl` 曾从未链接的 source worktree 运行，误创建空 Vercel project=`rebuy-release-0e5084b`；该项目 `latestDeployment=null`、`domains=[]`、deployment count=`0`。本地 `.vercel` 与 `.gitignore` 副作用已清理；删除该空外部项目的请求被安全门拒绝，必须待 Owner 在知情后明确批准，当前不得写成已删除。
