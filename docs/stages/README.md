...
状态枚举、证据枚举、Owner Gate 规则和生产边界以 [15 台账](../15-项目状态与阶段台账.md) 为准。GOV-1 已通过；G0/P1 当前为“已通过并冻结”，checkpoint 16 为通过；点击搜索已通过，直接 Enter 键盘提交未在本轮验证成功，转入后续专项。G1 当前为“执行中（G1.1 已完成，G1.2a 已完成，G1.2b 待 Owner Gate）”；G1.2a workflow/等价验证已完成，仍无 remote/远端 CI/Preview，G1.3 未开始；Exit Gate 未通过前不可进入 G2-A0。
