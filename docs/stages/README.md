...
| G2-A1 独立 Auth spike | 执行中（B1 最小连接及配置/能力只读预检、B2 本地安全基础及远端闭环、B2 external entry Gate design/preflight 已完成；E1 local bootstrap 已以 PR #15 merge commit 完成 main/远端交付闭环（仅代表 E1 本地隔离骨架）；E2a local email OTP 当前 REVIEW GO，E2b provider invite 与 P2-L local schema/RLS slice 均 CLOSED/待 Owner；两轮本地 start/status/health 已通过并完成 cleanup） | 本地静态 + Node 契约测试 + 构建 + 隔离首页 browser smoke + 本地 OTP/Mailpit/runtime harness + 远端 main merge/Actions + 外部资源只读摘要 + E1/E2a config/start/status/health/cleanup；健康结论限当次本地运行窗口 | [G2-A1 准备与资源门禁](./G2-A1-Auth-Spike准备与资源门禁.md)；[E2b 本地 Provider 邀请 Gate](./G2-A1-E2b-本地Provider邀请Gate.md)；[P2-L 本地 Schema 与 RLS 纵切 Gate](./P2-L-本地Schema与RLS纵切Gate.md)；[E2a local email OTP 证据](../evidence/G2-A1/2026-08-30-e2a-local-email-otp/README.md)；[E1 local bootstrap 证据](../evidence/G2-A1/2026-08-29-e1-local-bootstrap/README.md)；[B2 外部入口 Gate 证据](../evidence/G2-A1/2026-08-28-b2-external-entry-gate/README.md)；[B2 本地安全基础证据](../evidence/G2-A1/2026-08-28-b2-local-foundation/README.md)；[B1 配置/能力预检证据](../evidence/G2-A1/2026-08-28-b1-capability-preflight/README.md)；[B1 风险 Gate 证据](../evidence/G2-A1/2026-08-28-b1-risk-gate/README.md)；[Entry preparation 证据](../evidence/G2-A1/2026-08-28-entry-preparation/README.md)；[资源成本与密钥 Gate 模板](../templates/G2-A1-资源成本与密钥Gate模板.md)；[Auth 实测矩阵模板](../templates/G2-A1-Auth实测矩阵模板.md) |

GOV-1 的阶段事实见其[独立记录](./GOV-1-治理文档与状态台账.md)，当前状态已由 Owner 于 2026-08-25 18:58:57 CEST 确认为“已通过”；G0/P1 的路径/装饰标题、统一选择器和分类目录 IA 修订已完成，当前 G0/P1 为“已通过并冻结”。G1 已于 2026-08-27 完成 G1-19/G1 Exit=GO，验收 ref=`d51f1c7cb47e2fe2932b29bd39420f5d092a8160`；merge main 与 exact-head Actions 证据见[G1 final closeout](../evidence/G1/2026-08-27-g1-final-closeout/README.md)。G2-A0 当前为 `Exit GO；远端 docs-only reconciliation 已完成`，验收 ref=`140ea15d9c3f178a326709d35ad1750a156df0d1`，merge=`fd9b712c7b07bf34399f9838eebb75846425c1d1`；G2-A1 执行中，B1 最小连接及配置/能力只读预检、B2 本地安全基础及远端闭环、B2 external entry Gate design/preflight 已完成；PR #13 merge=`b3e53a71ca40729b139724a492e8d20afd02f341`，main Actions run=`33212054195`/job=`98987321203` success，`test:auth` 12 项且仅运行一次，exact merge 的 GitHub deployments=`0`，Vercel 无新增部署；该状态仅关闭 Gate 设计与本地基础远端交付，不等于 B2/Auth/G2-A1 整体通过；真实 Auth/OAuth/SMTP/session/DB/Storage/Production 与外部 action-time Owner Gate 继续 CLOSED；E1 local bootstrap 已以 PR #15 merge commit 完成 main/远端交付闭环，仅代表 E1 本地隔离骨架；独立安全审查 GO（P0=0/P1=0，唯一 P2 已修）；PR head=`fc6153b60872328b55b525730f3c653579ac2ea2`，merge=`a5e7fd1ae2ca7468610c0aab936121a27d124c02`；PR head CI run=`33256006999`/job=`99109715930` success；main exact merge CI run=`33256185497`/job=`99110205068` success；精确 merge 的 GitHub deployments=`0`；来源分支保留；Vercel 仍为 3 个既有 READY deployments（2 个 Preview、1 个 Production），本批无新增 deployment；E2–E5/hosted/deploy/Production CLOSED，详见 1.9 和 E1 evidence。当前状态必须与 [15 台账](../15-项目状态与阶段台账.md) 一致；本索引只做导航，不复制记录细节。
...
## 1.2 其他阶段计划记录（历史快照；当前 B2 见 1.6）

以下记录是旧时点对 G1/G2-A0 合同及其证据，以及 G2-A1 Entry preparation、资源预检和 B1 最小连接的历史快照；G1 已通过并关闭，G2-A0 已完成远端 docs-only reconciliation；当前 G2-A1/B2 状态见 1.6：
...
## 1.3 2026-08-28 G2-A1 B1 配置/能力只读预检（历史快照；当前 B2 见 1.6）
...
## 1.4 2026-08-28 G2-A1-B2 本地安全基础实现（本地 closeout 历史；当前外部入口 Gate 见 1.6）
...
## 1.5 2026-08-28 G2-A1-B2 本地安全基础远端闭环（历史快照；当前外部入口 Gate 见 1.6）
...
## 1.6 2026-08-28 G2-A1-B2 外部入口 Gate 设计与资源 preflight（历史快照；当前 E1 见 1.8）

- B2 external entry Gate design/preflight 已完成；这不是 B2/Auth 技术通过。当前 `main` 为 PR #13 docs-only closeout merge=`b3e53a71ca40729b139724a492e8d20afd02f341`，main Actions run=`33212054195`/job=`98987321203` success，`test:auth` 12 项且仅运行一次，exact merge 的 GitHub deployments=`0`；Vercel 仍 3 个既有 READY deployments，本批无新增部署。
- （历史 Gate 设计快照）E1 local bootstrap 仅在独立审查并合并后按条件打开：新 Rebuy local config、合成数据、55320–55329、缓存复用，不登录/link hosted project、不读取 hosted secret、不干预现有 SSH/容器；E2–E5、custom SMTP、hosted Auth/DB/Storage/Production/deploy 继续 CLOSED。详见[B2 外部入口 Gate 证据](../evidence/G2-A1/2026-08-28-b2-external-entry-gate/README.md)和[G2-A1 阶段记录](./G2-A1-Auth-Spike准备与资源门禁.md)。

## 1.7 2026-08-29 G2-A1 E1 local bootstrap 首轮失败诊断（历史；当前纠正见 1.8）

E1 local bootstrap 已完成 init/config/cache preflight；获批 image-download Gate 后 `supabase start` 的 Pull 阶段 13/13 均 skipped（本地已有），没有网络 pull/download/tag；随后因 Docker/Colima socket mount `operation not supported` 失败，项目限定 stop 成功，项目名容器、volume 和 55320–55329 TCP listener 均无输出。API/DB/Studio/Mailpit health 未形成通过证据。E2–E5、hosted Auth/DB/Storage/OAuth/SMTP、真实 Auth/OTP/invite、业务 schema、deploy、Production 继续 CLOSED。详见[E1 local bootstrap 证据](../evidence/G2-A1/2026-08-29-e1-local-bootstrap/README.md)。

## 1.8 2026-08-29 G2-A1 E1 local bootstrap 复验通过候选与 cleanup 纠正（历史候选；当前远端闭环见 1.9）

E1 当前状态为**本地完成候选/独立审查 GO，待 PR/CI/merge**。首轮按 help 逻辑排除名启动时仅出现 `analytics/storage/functions` 无效 warning，Pull=`6/6` 全部 local/skipped；第二轮改用实际有效名 `logflare/vector/realtime/storage-api/imgproxy/edge-runtime/gotrue/supavisor`，无 warning，Pull=`6/6` 全部 local/skipped。两轮实际集合均为 `db/kong/rest/pg_meta/studio/inbucket` 六个容器，published HostIP 全部 `127.0.0.1`；API REST=`200`、DB accepting、Studio=`307`、Mailpit=`200`。两轮 `stop --project-id ... --no-backup`、精确 network 删除和最终项目 containers/volumes/network/55320–55329 释放均通过；E2–E5、hosted Auth/DB/Storage/OAuth/SMTP、业务 schema、deploy、Production 继续 CLOSED。未修改全局 Colima/socket/symlink/runtime；详见[E1 local bootstrap 证据](../evidence/G2-A1/2026-08-29-e1-local-bootstrap/README.md)。

## 1.9 2026-08-29 G2-A1 E1 local bootstrap 远端交付闭环（当前）

- 当前 closeout 分支/提交为 docs-only；E1 local bootstrap 已以 PR #15 用 merge commit 合并 `main`，完成 PR #15 的 E1 config + evidence 远端交付闭环（包含 `supabase/.gitignore`、`supabase/config.toml`、`supabase/seed.sql` 与文档）；该结果仅代表 E1 本地隔离骨架，不代表 E2 或 hosted/Production 能力已打开。
...
## 1.10 2026-08-30｜E2a、E2b 与 P2-L 当前 Gate 导航（追加纠正）

- E2a local email OTP 当前为 REVIEW GO；证据仅限 local GoTrue/Mailpit、`@rebuy.test` synthetic-only 和既有本地 runtime 结果，不代表 G2-A1 整体通过。
...
状态枚举、证据枚举、Owner Gate 规则和生产边界以 [15 台账](../15-项目状态与阶段台账.md) 为准。GOV-1 已通过；G0/P1 当前为“已通过并冻结”；G1 已于 2026-08-27 以 G1-19/G1 Exit=GO 完成，ref=`d51f1c7cb47e2fe2932b29bd39420f5d092a8160`。G2-A0 已通过（Exit GO；远端 docs-only reconciliation 已完成，merge=`fd9b712c7b07bf34399f9838eebb75846425c1d1`），七项 Owner 政策已采纳；G2-A1 执行中，B1 最小连接及配置/能力只读预检、B2 本地安全基础及远端闭环、B2 external entry Gate design/preflight 已完成；PR #13 merge=`b3e53a71ca40729b139724a492e8d20afd02f341`，main Actions run=`33212054195`/job=`98987321203` success，`test:auth` 12 项且仅运行一次，exact merge 的 GitHub deployments=`0`，Vercel 无新增部署；该状态仅关闭 Gate 设计与文档/本地基础远端交付，不等于 B2/Auth/G2-A1 整体通过；真实 Auth/OAuth/SMTP/session/DB/Storage/Production 与外部 action-time Owner Gate 继续 CLOSED；E1 local bootstrap 已以 PR #15 merge commit 完成 main/远端交付闭环，仅代表 E1 本地隔离骨架；独立安全审查 GO（P0=0/P1=0，唯一 P2 已修）；E2–E5/hosted/deploy/Production CLOSED；下一步仅可进入 E2 action-time Gate 评估，不代表 E2 已打开或可执行。详见[G1 final closeout](../evidence/G1/2026-08-27-g1-final-closeout/README.md)、[G2-A0 阶段记录](./G2-A0-账号安全合同与威胁模型验收.md)、[G2-A1 B2 外部入口 Gate 证据](../evidence/G2-A1/2026-08-28-b2-external-entry-gate/README.md)、[G2-A1 B2 本地安全基础证据](../evidence/G2-A1/2026-08-28-b2-local-foundation/README.md)、[G2-A1 B1 风险 Gate 证据](../evidence/G2-A1/2026-08-28-b1-risk-gate/README.md)。

## 3.1 2026-08-28 当前状态纠正

以 [15 台账](../15-项目状态与阶段台账.md) 为唯一当前状态源：G2-A0 已通过且远端 docs-only reconciliation 已完成（merge=`fd9b712c7b07bf34399f9838eebb75846425c1d1`）；G2-A1 执行中，B1 最小连接及配置/能力只读预检、B2 本地安全基础及远端闭环、B2 external entry Gate design/preflight 已完成；PR #13 merge=`b3e53a71ca40729b139724a492e8d20afd02f341`，main Actions run=`33212054195`/job=`98987321203` success，`test:auth` 12 项且仅运行一次，exact merge 的 GitHub deployments=`0`，Vercel 无新增部署；该状态仅关闭 Gate 设计与文档/本地基础远端交付，不等于 B2/Auth/G2-A1 整体通过；真实 Auth/OAuth/SMTP/session/DB/Storage/Production 与外部 action-time Owner Gate 继续 CLOSED；E1 local bootstrap 已以 PR #15 merge commit 完成 main/远端交付闭环，仅代表 E1 本地隔离骨架；独立安全审查 GO（P0=0/P1=0，唯一 P2 已修）；E2–E5/hosted/deploy/Production CLOSED；下一步仅可进入 E2 action-time Gate 评估，不代表 E2 已打开或可执行。完整 resource/cost/secret Gate 关闭。后续四批合同、官方来源刷新、复用预检与资源责任字段见[G2-A1 准备与资源门禁](./G2-A1-Auth-Spike准备与资源门禁.md)，静态摘要见[B2 外部入口 Gate 证据](../evidence/G2-A1/2026-08-28-b2-external-entry-gate/README.md)、[B2 本地安全基础证据](../evidence/G2-A1/2026-08-28-b2-local-foundation/README.md)和[B1 风险 Gate 证据](../evidence/G2-A1/2026-08-28-b1-risk-gate/README.md)。本索引不复制运行时或生产结论。
