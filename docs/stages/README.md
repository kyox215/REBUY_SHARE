...
| G2-A1 独立 Auth spike | 执行中（E2a local email OTP REVIEW GO；注册/登录 backend runtime PASS、浏览器 UI 因无实例待补；E2b provider invite CLOSED；P2-L schema/RLS attempt #16 RUNTIME PASS、独立审查待完成） | 本地静态 + Node/Auth 契约 `40/40` + local 注册/登录/退出/Mailpit runtime + 构建 + 历史浏览器 smoke + P2-L pgTAP `87/87` + strict lint/advisors/migration list/cleanup；均不外推为 hosted/Production | [G2-A1 准备与资源门禁](./G2-A1-Auth-Spike准备与资源门禁.md)；[注册/登录 HTTP PASS](../evidence/G2-A1/2026-09-03-registration-login-http-attempt-2-pass/README.md)；[注册/登录 browser attempt #1](../evidence/G2-A1/2026-09-03-registration-login-browser-attempt-1/README.md)；[P2-L 本地 Schema 与 RLS 纵切 Gate](./P2-L-本地Schema与RLS纵切Gate.md)；[P2-L attempt #16](../evidence/P2-L/2026-09-03-schema-runtime-attempt-16/README.md)；[E2a local email OTP 证据](../evidence/G2-A1/2026-08-30-e2a-local-email-otp/README.md)；[E1 local bootstrap 证据](../evidence/G2-A1/2026-08-29-e1-local-bootstrap/README.md) |

GOV-1、G0/P1、G1、G2-A0 与 E1 的历史完成/远端闭环事实仍以各自 evidence 和 [15 台账](../15-项目状态与阶段台账.md) 为准。当前 G2-A1 仍在执行中：E2a local email OTP 为 REVIEW GO；注册/登录新候选 Auth contract `40/40` 且 local backend runtime 已通过，但 active browser Gate 在业务请求前因无浏览器实例停止，UI PASS 仍待补；E2b provider invite 保持 CLOSED。P2-L attempt #16 已建立 schema/RLS/runtime PASS（pgTAP `87/87`、strict lint、security/performance advisors、migration list 与 cleanup），但完整 P2-L Exit 仍待绑定 exact candidate 的独立审查。该状态不等于完整 P2、G2-A1、hosted 或 Production 通过；P3–P7、main push/deploy 继续关闭。本索引只做导航，不复制详细记录。
