...
| G1-11 | Preview 项目/owner/ref/Node/root `prototype`/访问/变量/health/日志/停止入口 | 预检完成/实施缺失 | [G1.3-1 provider/Preview preflight](../evidence/G1/2026-08-27-g1-3-1-provider-preview-preflight/README.md)；[G1.3-2 execution review](../evidence/G1/2026-08-27-g1-3-2-preview-execution-review/README.md)；[G1.3-0 预检](../evidence/G1/2026-08-26-g1-3-0-local-environment-preflight/README.md) | provider/project/owner/access 已只读确认，G1.3 修正版 Owner Gate 与 external preflight/rehearsal 已通过；`engines.node=22.x` 覆盖 project Node24，默认不 PATCH，build log 实证 Node22、cost、Preview、health、日志、停止入口仍待实施确认；provider `rootDirectory=prototype`，CLI 必须从精确 archive 仓库根执行，禁止从 archive/prototype 执行 |
...
2. 只允许已通过 CI 的可追溯 ref 建立 Preview；provider `rootDirectory=prototype` 保持不变，从精确 archive 的仓库根执行 CLI，禁止从 archive 的 `prototype/` 子目录执行以免 `prototype/prototype`；Node 22 必须由 build log 实证；G2-A1 前不得注入 Auth/DB 值或生产变量。
...
- `package.json` `engines.node=22.x` 覆盖 project Node24，正式策略为默认不 PATCH；未来 Preview 必须从 exact git archive 的仓库根执行 CLI，禁止从 archive/prototype 执行以免 `prototype/prototype`，并以每次 build log 实证 Node22。
