...
当前清单证据：本地静态、G1.2a 本地 workflow/等价验证、G1.3-0 本地预检、G1.3-1 provider/CLI/隔离预检、G1.3-2 高风险 Preview 执行独立审查与 2026-08-27 10:33:59 CEST Owner Gate 授权记录、G1.2b 真实 PR/Actions、PR #1 main merge 与远端只读设置核验；bad503 Preview 已真实完成并有脱敏 READY/health 证据，bad PR #4 保持 Draft，bad ref、main 与 Production 均未变；good Preview deployment `dpl_D2oNMJhvQsvbbyszgApm24aGLYnZ` 已 READY（project=`rebuy-share`、Target=Preview），deployment provider-resolved Node22；`/`=200、`/api/health/app`=200、body 仅 `{"status":"healthy"}`、Cache-Control 含 `no-store`；Protection=`all_except_custom_domains`、Preview env=0；deployment 列表仅 good+bad+old Production，Production latest/alias count=2/fingerprint 不变量未变；旧 5ce 首轮代码候选快照及 run=`33074662873` / job=`98525606734` 安装、typecheck、lint、build 已成功；good Preview source ref=`f4225397dc6c6b99e315d5ca4a7ecbc8695fb529`（当时 PR #5 current head）的 exact-head Actions run=`33078824609` / job=`98540116896` 的 install/typecheck/lint/build 已成功，随后部署上述 good Preview；本次 docs-only closeout 将生成后继 head，其 SHA/run 不预写、不递归回写；PR current-head 及 docs-only closeout 后新 Actions 仍待实时核验；bad→good online recovery 技术 GO；final Preview 默认不重复（docs-only 且 code/config/lock 未变，上限非强制）；G1 Exit/G1-19 签署仍缺，当前结论保持 `NO-GO`
...
- 当前 G1 状态必须以 15 为准：G1.1 已完成，G1.2a 已完成，G1.2b 真实 PR/CI 已完成并通过 PR #1 merge commit 合并 `main`；main push 的 install/typecheck/lint/build 已成功，`integration/g1-2b` 仍保留；G1.3-0 本地预检与 G1.3-1 provider/CLI/部署前隔离预检已完成，G1.3-2 高风险独立审查已完成（原方案 NO-GO，修正后条件 GO 候选），Owner 于 2026-08-27 10:33:59 CEST 已批准修正版 G1.3 受控范围，bad503 Preview 已真实完成并有脱敏 READY/health 证据；good Preview deployment `dpl_D2oNMJhvQsvbbyszgApm24aGLYnZ` 已 READY（project=`rebuy-share`、Target=Preview），deployment provider-resolved Node22；`/`=200、`/api/health/app`=200、body 仅 `{"status":"healthy"}`、Cache-Control 含 `no-store`；Protection=`all_except_custom_domains`、Preview env=0；deployment 列表仅 good+bad+old Production，Production latest/alias count=2/fingerprint 不变量未变；首轮代码候选快照 ref=`5ce3723b73edcd7284f88b26d6faa0e31ed01b40`（首轮本地验证已通过；首轮时为 PR #5 head，当时 PR #5 为 OPEN/普通非 Draft），首轮 Actions run=`33074662873` / job=`98525606734` 的 install/typecheck/lint/build 已成功；本次 docs-only closeout 将生成后继 head，其 SHA/run 不预写、不递归回写；PR current-head 及 docs-only closeout 后新 Actions 仍待实时核验；bad→good online recovery 技术 GO；final Preview 默认不重复（docs-only 且 code/config/lock 未变，上限非强制）；G1 Exit/G1-19 签署仍缺，G2-A0 不打开。
...
| G1-05 | 本地 `typecheck`、`lint`、`build` 通过 | 已满足（远端/Preview） | [G1.2a 证据](../evidence/G1/2026-08-26-g1-2a-local-workflow/README.md)；[G1.2b 远端证据](../evidence/G1/2026-08-27-g1-2b-remote-ci/README.md)；[G1.3-3 good200 evidence](../evidence/G1/2026-08-27-g1-3-3-good200-local-candidate/README.md) | good source exact-head Actions 四步已成功；current-head 新 Actions 与 Owner G1-19 仍待 |
| G1-06 | 最小 workflow、失败停止、`contents: read`、action 完整 SHA 固定 | 已满足（远端） | `.github/workflows/prototype-quality.yml`；[G1.2a 证据](../evidence/G1/2026-08-26-g1-2a-local-workflow/README.md)；[G1.2b 远端证据](../evidence/G1/2026-08-27-g1-2b-remote-ci/README.md) | workflow 静态边界与真实 PR check 已证；good Preview 不新增 runner/workflow 要求，current-head 新 Actions 与 Owner G1-19 仍待；仓库允许 actions all、SHA enforcement false 为治理风险 |
