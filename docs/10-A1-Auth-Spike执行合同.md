...
文档状态：**G2-A1 技术阶段尚未开始；最小资源存在性/基础预检已完成；独立安全复审已完成、首轮 finding 已关闭；Owner/主代理批准打开 A1-B1 最小技术范围（已授权/待执行）；完整 resource/cost/secret/Auth/DB/Storage/OAuth/SMTP/真实 PII/部署/Production Gate 关闭**；G2-A0 Exit GO，远端 docs-only reconciliation 已完成；七项 Owner 政策已采纳；即使资源已存在，也须另行完成 Auth/secret/DB 等技术 Gate 后才可进入独立测试
适用范围：验证 Rebuy 三入口认证、OAuth callback、identity linking、邀请邮箱控制权、MFA、会话和最小服务端集成边界  
当前允许：A1-B1 最小技术范围已授权但待执行：仅可只读取得 project URL + modern publishable key，确认 `.gitignore` 阻止 env，将原值仅存入 gitignored local env，复用/定向扩展现有 SSR/client/health 做 EU non-production synthetic-only 连接验证，并记录 Free 限制、STOP/cleanup；不得以此改变已冻结的 G0/P1 UI。
...
- 当前正式状态为：**G2-A1 技术阶段未开始；最小资源存在性/基础预检已完成；独立安全复审已完成、首轮 finding 已关闭；Owner/主代理批准打开 A1-B1 最小技术范围（已授权/待执行）；完整 resource/cost/secret/Auth/DB/Storage/OAuth/SMTP/真实 PII/部署/Production Gate 关闭**。无资源 Entry preparation 已归档；本次窄范围预检仅核对独立 Free 组织、项目、区域、provider quote 确认和管理面健康状态；B1 连接动作尚待执行，不预写 Auth/DB/Storage/OAuth/SMTP 结果，不读取 secret/env/PII，不部署或修改 Production。
...
本节是 A1-B1 的最小风险 Gate 与复用预检记录。独立安全复审已完成、首轮 finding 已关闭，Owner/主代理批准打开 A1-B1 最小技术范围；该授权仍待执行，不代表 G2-A1 Auth/B1 技术通过。该批按关键风险路由执行：`luna_worker / max`、默认一个执行代理。
...
| 下一批可申请动作 | 复核当前官方 docs/changelog；只读取得 project URL + modern publishable key；确认 `.gitignore` 阻止 env；复用或定向扩展现有 SSR client 与 health route；在 EU non-production local synthetic-only 连接中验证不依赖 service_role；记录 Free limitations、STOP、expiry/cleanup 和回退 |
...
上述“下一批可申请动作”现为已授权但待执行的 B1 最小动作清单，不是 broad waiver（全局豁免），也不等于 G2-A1 Auth/B1 技术通过。任何实际连接成功、配置成功或技术通过都必须在后续批次以实时证据记录；本节不预写结果。A1 通过也不自动打开 B2/B3、P2 或 Production。
...
出现非零费用、add-on/upgrade、生产或真实 PII 连接、service_role 依赖、secret/URL/key 进入仓库/日志/证据、无法证明 `.gitignore` 或环境隔离、health 错误泄露、无法清理或其他越界，立即停止并保留脱敏失败摘要；不得绕过 Gate 继续。B1 技术结果需回写阶段证据并由独立审查复核。
