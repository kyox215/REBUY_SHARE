...
## 27. 2026-08-28｜G2-A1-B2 外部入口 Gate 设计与资源 preflight（历史快照；当前 E1 远端闭环见第 30 节）
...
- 第 28 节仅保留首次未排除 Vector 的 Docker/Colima mount 失败诊断；binary-derived image refs 仅为首次 cache 诊断，非当前运行时权威事实。本批未修改全局 Colima、Docker socket、symlink 或 host runtime。
- 两轮差异：首轮按 `start --help` 逻辑名排除，CLI 警告 `analytics/storage/functions` 无效；Pull=`6/6` 全部 local/skipped，健康和 cleanup 通过。第二轮按实际有效容器名排除 `logflare/vector/realtime/storage-api/imgproxy/edge-runtime/gotrue/supavisor`；无 warning，Pull=`6/6` 全部 local/skipped，得到相同六个目标容器集合并复验通过。两轮均未 image pull/download/tag。
...
- 当前状态为 **E1 local bootstrap 本地完成候选/待独立审查**；E2–E5、hosted Auth/DB/Storage/OAuth/SMTP、真实 Auth/OTP/invite、业务 schema、deploy、Production 继续 **CLOSED**。未执行真实 Auth/DB/Storage 写入、hosted 登录/link、费用、push、PR、merge 或 deploy。
