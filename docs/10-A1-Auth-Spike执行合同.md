...
文档状态：A0 通过后才能连接独立测试环境  
适用范围：验证 Rebuy 三入口认证、OAuth callback、identity linking、邀请邮箱控制权、MFA、会话和最小服务端集成边界  
当前允许：A0 通过前可以继续做无后端、无真实账号、无真实 PII 的 UI 原型准备。  
当前不允许：不得把 A1 标记为“技术验证通过”，不得连接 production、真实业务数据、真实客户邮箱或真实证件。  
...
A1 不以静态一次性码为恢复路径。若供应商界面或 API 出现相近命名，必须记录其实际语义、生命周期、存储和安全风险，不能把它当作本合同的备用因子替代。
